...
void scene3() {    //10000 cubes
	...
	for (int x = 0; x < 100; x++) {
		for (int y = 0; y < 100; y++) {
			...
			*m = Mesh::makeCube(1.f);
			scene.push_back(m);
			float px = -20.f + (y * 2.0f);
			float py = 20.f - (x * 2.0f);
			float pz = -20.f - x * 0.2f;
...
void RenderMeshBlock(Renderer& renderer, const std::vector<Mesh*>& block, matrix& camera, Light& L) {
	for (auto& m : block) {
		render(renderer, m, camera, L);
	}
}

void MtRender(Renderer& renderer, const std::vector<Mesh*>& scene, matrix& camera,  Light& L, int Threads) {
	std::vector<std::thread> threads;
	size_t All_Meshes = scene.size();
	size_t meshesPerThread = (All_Meshes + Threads - 1) / Threads;
	for (int i = 0; i < Threads; i++) {
		size_t start = i * meshesPerThread;
		size_t end = min(start + meshesPerThread, All_Meshes);
		if (start >= end)
			break;
		std::vector<Mesh*> threadMeshes(scene.begin() + start, scene.begin() + end);
		threads.emplace_back(RenderMeshBlock, std::ref(renderer), threadMeshes, std::ref(camera), std::ref(L));
	}
	for (auto& t : threads) {
		t.join();
	}
...
void scene3Mt() {
	...
	*sphere = Mesh::makeSphere(1.0f, 10, 20);
	...
	while (running) {
		...
		if (renderer.canvas.keyPressed(VK_ESCAPE)) break;
		MtRender(renderer, scene, camera, L, Max_Threads);
		renderer.present();
	}
	for (auto& m : scene) {
		delete m;
